...
import { Direction, EventType } from '../src/rec/Ride'
import { Position } from '../src/shared'

describe('Replay', () => {
  test('parses valid replay #1', async () => {
    const replay = await Replay.load('__tests__/assets/replays/rec_valid_1.rec')
  ...
  test('parses valid multi replay #1', async () => {
    const replay = await Replay.load('__tests__/assets/replays/rec_valid_2.rec')
    expect(replay).toBeInstanceOf(Replay)
